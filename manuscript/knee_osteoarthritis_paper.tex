...
In addition to the core 8,260-image classification set, the Mendeley repository includes a supplementary benchmark partition of 1,526 detector-localized knee joint crops (\texttt{auto\_test}) derived from 811 subjects belonging strictly to the primary held-out test cohort ($\mathcal{P}_{\text{Auto}} \subset \mathcal{P}_{\text{Test}}$) \cite{chen2018dataset, chen2019fully}. In the benchmark dataset release, these crops were extracted using an automated YOLO-based knee joint detector trained to localize knee joint margins from full bilateral radiographs, introducing localized boundary shifts and field-of-view perturbations. We evaluated this supplementary partition under verified zero-training-leakage conditions ($|\mathcal{P}_{\text{Train}} \cap \mathcal{P}_{\text{Auto}}| = 0$) to measure model resilience against automated detection noise.
...
\vspace{2pt}
\parbox{\columnwidth}{\footnotesize $^*$Auto-Test images represent automated YOLO-based knee joint detector crops provided within the Mendeley benchmark repository \cite{chen2018dataset, chen2019fully}, derived from 811 subjects belonging strictly to the primary held-out test cohort ($\mathcal{P}_{\text{Auto}} \subset \mathcal{P}_{\text{Test}}$). They maintain verified patient-level isolation from training and validation partitions ($|\mathcal{P}_{\text{Train}} \cap \mathcal{P}_{\text{Auto}}| = |\mathcal{P}_{\text{Val}} \cap \mathcal{P}_{\text{Auto}}| = 0$).}
\end{table}
