...
\item \textbf{Reproducibility Protocol and Checkpoint Selection:} Global random seeds were locked across Python \texttt{random}, NumPy, PyTorch, and CUDA backend (\texttt{torch.backends.cudnn.deterministic = True}) using seed 42. Terminal model weights were selected strictly based on the highest validation Quadratic Weighted Kappa (QWK) on $\mathcal{P}_{\text{Val}}$. While seed 42 was fixed for primary benchmark reproducibility, five-fold cross-validation across multiple random seeds confirmed model stability against partition stochasticity (mean primary test accuracy $84.82 \pm 0.45\%$, mean QWK $0.743 \pm 0.012$ under TTA).
\end{itemize}
...
The ablation confirms the necessity of distance-aware optimization: standard cross-entropy achieves 81.28\% accuracy and 0.678 QWK, but incurs 2 catastrophic non-adjacent reversals (Severe misdiagnosed as Low/No OA). Incorporating the distance penalty ($\lambda_{\text{ord}} = 0.50$) improves accuracy by +2.16 pp to 83.44\%, elevates QWK by +0.043 to 0.721, achieves 100\% Severe recall on held-out test data, and completely eliminates extreme grade reversals. The $\lambda_{\text{ord}} = 0.50$ configuration, selected on the validation partition ($\mathcal{P}_{\text{Val}}$) prior to terminal testing, provided the optimal validation trade-off between cross-entropy discrimination and ordinal agreement, and was therefore frozen for the primary test evaluation. The post-hoc results in Table~\ref{tab:loss_ablation} confirm that this validation-selected parameter generalizes robustly to the primary held-out test partition; setting $\lambda_{\text{ord}} = 1.00$ slightly over-penalizes metric distance at the expense of nominal boundary precision (82.61\% accuracy).
