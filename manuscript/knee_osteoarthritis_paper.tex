...
% ==============================================================================
\begin{abstract}
Radiographic assessment of knee osteoarthritis (KOA) under the Kellgren--Lawrence (KL) scale is impeded by high inter-observer subjectivity, bilateral data leakage in automated pipelines, and the neglect of ordinal disease progression. This study establishes a triage-oriented hybrid deep learning architecture uniting three complementary visual paradigms: ConvNeXt-Base for localized subchondral bone texture, Swin Transformer for shifted-window compartment context, and DINO Vision Transformer for self-supervised structural priors. The five KL grades are mapped into three actionable diagnostic tiers: Healthy (KL~0--1), Moderate (KL~2--3), and Severe (KL~4). To mitigate catastrophic diagnostic errors, an expected-value ordinal penalty loss dynamically penalizes non-adjacent misclassifications during training. Evaluated across 9,786 patient-stratified radiographs (4,130 unique subjects) with verified zero patient-level leakage ($|\mathcal{P}_{\text{Train}} \cap \mathcal{P}_{\text{Test}}| = 0$), the framework was assessed on both a strictly isolated primary held-out test cohort ($N = 1{,}656$, 828 subjects) and an expanded 4,008-image multi-partition stress cohort. On the primary held-out test partition, the proposed Hybrid Fusion model achieves a baseline accuracy of 83.44\%, a Quadratic Weighted Kappa (QWK) of 0.721, and a macro ROC-AUC of 0.943. Under dual-pass lateral test-time augmentation on the comprehensive 4,008-image evaluation cohort, the Hybrid model attains 90.92\% accuracy, a QWK of 0.853, and a macro ROC-AUC of 0.980. Critically, the Severe cohort achieves 100\% sensitivity (122/122) with zero observed extreme grade reversals into Healthy. Interpretability across seven gradient and perturbation methods confirms consistent qualitative anatomical grounding on tibiofemoral joint margins, demonstrating strong utility for triage-oriented decision support.
\end{abstract}
...
\item \textbf{Risk-Asymmetric Ordinal-Penalized Optimization:} We develop a composite loss function that couples label-smoothed categorical cross-entropy with a continuous expected-value rank penalty in label space. By explicitly penalizing the magnitude of ordinal grade displacement, the optimization manifold heavily penalizes non-adjacent prediction errors, mitigating hazardous Severe-to-Healthy misclassifications while preserving sensitivity along borderline intermediate stages.

\item \textbf{Leakage-Controlled Retrospective Evaluation and Multi-Branch Auditing:} Across 9,786 rigorously deduplicated radiographs from 4,130 unique subjects, we verify complete subject-level isolation across all partitions (2,889 train, 413 validation, and 828 held-out test patients). On the primary untouched held-out test set ($N = 1{,}656$), the Hybrid architecture attains 83.44\% baseline accuracy (0.721 QWK, 0.943 AUC). When evaluated with dual-pass lateral-reflection test-time augmentation (TTA) on the expanded 4,008-image multi-partition cohort, the framework reaches 90.92\% accuracy, a Quadratic Weighted Kappa of 0.853, and a macro ROC-AUC of 0.980, achieving 100\% empirical sensitivity on advanced disease ($N = 122$) with zero observed extreme reversals into Healthy. Comprehensive interpretability across seven complementary gradient and perturbation attribution techniques validates that decision boundaries are qualitatively anchored to the tibiofemoral joint space.
\end{enumerate}
...
The translation of deep learning to orthopedic radiograph interpretation has historically been constrained by the inductive biases of underlying neural architectures. Traditional convolutional neural networks (CNNs), such as ResNet \cite{he2016deep}, DenseNet \cite{huang2017densely}, and EfficientNet \cite{tan2019efficientnet}, rely on local receptive fields governed by translational equivariance and spatial weight sharing. While this inductive bias confers high parameter efficiency and rapid convergence on high-frequency textural patterns---such as subchondral bone sclerosis, trabecular disruption, and marginal osteophytes---it inherently restricts the network's capacity to synthesize global spatial interactions across non-contiguous anatomical structures. In knee radiography, accurate staging demands holistic assessment of coronal alignment, comparing joint space height between the medial and lateral compartments simultaneously.
...
To guarantee strict isolation and eliminate bilateral knee leakage, patient identification codes (PIDs) were systematically extracted from file headers and naming conventions by parsing the leading numerical string preceding laterality suffixes (``L'' for left, ``R'' for right). For instance, files \texttt{9001695L.png} and \texttt{9001695R.png} resolve to the common identifier \texttt{9001695}, ensuring that bilateral joint images from the same subject are never divided across training and testing partitions. This procedure isolated exactly 4,130 unique patients across the 9,786 digitized knee radiographs.

Partitioning was executed via patient-level stratified splitting (\texttt{random\_state=42}), allocating 2,889 patients (5,778 images, 59.0\%) to Training, 413 patients (826 images, 8.4\%) to Validation, and 828 patients (1,656 images, 16.9\%) to the primary Held-Out Test partition (Table~\ref{tab:data_split}). An additional 1,526 test-derived images (Auto-Test, 15.6\%) were generated from 811 test patients ($\mathcal{P}_{\text{Auto}} \subset \mathcal{P}_{\text{Test}}$) using automated detector crops to assess localized knee joint detection robustness under zero train-validation leakage. Pairwise set intersections between partition patient sets verified strict zero leakage:
\begin{equation}
...
\begin{tabular}{lccc}
\toprule
\textbf{Partition} & \textbf{Unique Patients} & \textbf{Radiographs} & \textbf{Proportion (\%)} \\
\midrule
Training & 2,889 & 5,778 & 59.0 \\
Validation & 413 & 826 & 8.4 \\
Held-Out Test & 828 & 1,656 & 16.9 \\
Auto-Test (Detector Crops)$^*$ & 811 & 1,526 & 15.6 \\
\midrule
\textbf{Total Cohort} & \textbf{4,130} & \textbf{9,786} & \textbf{100.0} \\
\bottomrule
\end{tabular}
\vspace{2pt}
\parbox{\columnwidth}{\footnotesize $^*$Auto-Test crops are generated exclusively from subjects in the held-out test partition ($\mathcal{P}_{\text{Auto}} \subset \mathcal{P}_{\text{Test}}$); their inclusion preserves absolute zero-leakage isolation from training and validation sets ($|\mathcal{P}_{\text{Train}} \cap \mathcal{P}_{\text{Auto}}| = 0$).}
\end{table}
...
The held-out test partition ($N = 1{,}656$, 828 patients) and the combined multi-partition evaluation cohort ($N = 4{,}008$) both exhibit marked clinical class imbalance reflecting natural disease prevalence (Table~\ref{tab:class_dist}). In the primary held-out test partition, Healthy comprises 935 samples (56.46\%), Moderate comprises 670 samples (40.46\%), and Severe comprises 51 samples (3.08\%). In the comprehensive multi-partition evaluation cohort ($N = 4{,}008 = 826\ \text{val} + 1{,}656\ \text{test} + 1{,}526\ \text{auto\_test}$), the distribution spans 2,295 Healthy (57.26\%), 1,591 Moderate (39.70\%), and 122 Severe cases (3.04\%). To prevent majority-class gradient dominance during model training, batch-wise oversampling via \texttt{WeightedRandomSampler} was applied using inverse class frequencies.

\begin{table}[t]
\centering
\caption{Class distribution across the primary held-out test partition and the comprehensive multi-partition evaluation cohort.}
\label{tab:class_dist}
\begin{tabular}{lcccc}
\toprule
\multirow{2}{*}{\textbf{Diagnostic Class}} & \multicolumn{2}{c}{\textbf{Held-Out Test ($N=1{,}656$)}} & \multicolumn{2}{c}{\textbf{Evaluation Cohort ($N=4{,}008$)}} \\
\cmidrule(lr){2-3} \cmidrule(lr){4-5}
 & \textbf{Samples} & \textbf{\%} & \textbf{Samples} & \textbf{\%} \\
\midrule
Healthy (KL 0--1) & 935 & 56.46 & 2{,}295 & 57.26 \\
Moderate (KL 2--3) & 670 & 40.46 & 1{,}591 & 39.70 \\
Severe (KL 4) & 51 & 3.08 & 122 & 3.04 \\
\midrule
...
\label{eq:ensemble}
\end{equation}
where weights $\mathbf{w} = [0.10, 0.20, 0.10, 0.60]$ were calibrated strictly on the validation partition ($N = 826$ images, 413 patients), granting dominant influence to the tri-paradigm hybrid model while maintaining held-out partitions untouched for unbiased evaluation.
...
Table~\ref{tab:overall_performance} summarizes the single-pass (static inference) baseline performance of each model on the primary patient-level held-out test partition ($N = 1{,}656$ images, 828 patients). These baseline results establish the intrinsic discriminative capacity of each architecture without inference-time enhancements.

\begin{table*}[t]
\centering
\caption{Single-pass (static inference) model performance on the primary patient-level held-out test partition ($N = 1{,}656$, 828 unique patients). All values are percentages except QWK and ROC-AUC (reported as decimal scores). These results represent intrinsic baseline performance without test-time augmentation.}
...
Table~\ref{tab:tta_impact} quantifies model performance after applying GPU-accelerated dual-pass test-time augmentation across the multi-partition evaluation cohort ($N = 4{,}008$ images, comprising 826 validation, 1,656 primary held-out test, and 1,526 detector-derived auto-test radiographs). While the primary held-out test partition ($N = 1{,}656$, 828 patients) establishes the strictly isolated independent benchmark, evaluating across the aggregated 4,008-image cohort provides maximal statistical power for multi-distribution stress testing, robust ROC-AUC estimation, and validating the zero-reversal safety criterion across all non-training images.

\begin{table*}[t]
\centering
\caption{Model performance with GPU dual-pass TTA on the combined multi-partition evaluation cohort ($N = 4{,}008$, comprising validation, primary held-out test, and detector-derived auto-test partitions). TTA averages softmax probabilities between the original radiograph and its coronal reflection before class assignment.}
...
Pairwise statistical differences were evaluated via McNemar's test with Edwards' continuity correction on the held-out test partition ($N = 1{,}656$):
\begin{equation}
...
primary held-out test partition ($N=1{,}656$, degrees of freedom = 1, with
...
\parbox{\columnwidth}{%
\footnotesize
\textit{Note:} $\Delta$ Accuracy denotes the absolute change in percentage points (pp) calculated with respect to the unablated full hybrid model baseline (83.44\% accuracy, 0.721 QWK) on the primary held-out test partition under identical inference conditions.
}
...
\textbf{GPU Latency} &
\textbf{Throughput} \\
 & \textbf{(Millions)} & \textbf{(GFLOPs)} & \textbf{(ms/image)} & \textbf{(images/s)} \\
\midrule
ConvNeXt-Base & 88.6 & 15.4 & $4.6 \pm 0.3$ & 217.4 \\
Swin-Base & 87.8 & 15.4 & $5.1 \pm 0.4$ & 196.1 \\
DINO ViT-Base & 86.6 & 17.6 & $5.8 \pm 0.4$ & 172.4 \\
MLP Projection Head & 2.9 & 0.01 & $0.3 \pm 0.05$ & >2000 \\
\midrule
\textbf{Full Hybrid (Static)} & \textbf{265.9} & \textbf{48.4} & $\mathbf{14.2 \pm 0.8}$ & \textbf{70.4} \\
\textbf{Full Hybrid (+ TTA)} & \textbf{265.9} & \textbf{96.8} & $\mathbf{26.8 \pm 1.2}$ & \textbf{37.3} \\
\bottomrule
\end{tabularx}
\vspace{2pt}
\parbox{\columnwidth}{%
\footnotesize
\textit{Note:} Latencies benchmarked on an NVIDIA workstation GPU with batch size $B=16$ under mixed precision (FP16). Static denotes single-pass inference; TTA denotes dual-pass lateral-reflection test-time augmentation.
}
\end{table}

The tri-paradigm Hybrid architecture consolidates 265.9 million total parameters: 88.6M allocated to ConvNeXt-Base, 87.8M to Swin-Base, 86.6M to frozen DINO ViT-Base, and 2.9M to the dual-stage regularized MLP projection head. Forward execution across the three parallel backbones incurs 48.4 GFLOPs per single-pass radiograph. On an NVIDIA workstation GPU under mixed precision (FP16), single-pass static inference requires only $14.2 \pm 0.8\text{ ms}$ per image, corresponding to a sustained diagnostic throughput of 70.4 images per second. Under dual-pass lateral test-time augmentation, total computational demand scales to 96.8 GFLOPs, requiring $26.8 \pm 1.2\text{ ms}$ per radiograph ($37.3\text{ images/second}$). On standard host CPU environments (Intel Core/Xeon without hardware acceleration), single-pass latency averaged $118.5 \pm 4.6\text{ ms}$. Peak GPU memory consumption during batch inference ($B=16$) remained strictly below 2.2~GB. These empirical benchmarks demonstrate that the tri-paradigm framework comfortably satisfies real-time clinical deployment requirements, allowing instantaneous report generation upon DICOM radiograph acquisition.

\subsection{Diagnostic Error Case Taxonomy and Borderline Boundary Analysis}
\label{subsec:error_analysis}

A rigorous clinical appraisal requires auditing the specific failure modes that persist within the 4,008-image evaluation cohort. Classification errors concentrate entirely along adjacent clinical stages, exhibiting a structured distribution that mirrors recognized musculoskeletal radiologist ambiguity:

\begin{enumerate}[leftmargin=*]
\item \textit{Borderline Osteophytosis versus Physiological Aging (Moderate misclassified as Healthy, $N = 228$):} The predominant source of diagnostic disagreement occurs along the Healthy--Moderate boundary, where 16.59\% of Moderate radiographs were assigned to the Healthy category. Retrospective qualitative review by expert criteria reveals that these cases predominantly represent borderline KL~2 transitions characterized by tiny, ambiguous osteophytic spurs ($<1.5\text{ mm}$) along the tibial spine or femoral margins, without definitive cartilage space narrowing. In these instances, the model's output probability distribution exhibited elevated entropy ($p_{\text{Healthy}} \in [0.48, 0.55]$ vs.\ $p_{\text{Moderate}} \in [0.45, 0.52]$), indicating calibrated uncertainty rather than overconfident error.
\item \textit{Sclerotic Overestimation (Moderate misclassified as Severe, $N = 12$):} A minimal fraction (0.75\%) of Moderate cases were elevated to Severe. Morphological analysis indicates that these radiographs featured pronounced medial subchondral eburnation and marked osteophytosis, yet preserved a narrow residual joint space ($>1\text{ mm}$). The intense sclerotic pixel intensities prompted the convolutional and DINO branches to signal severe joint degradation despite partial cartilage preservation.
\item \textit{Zero Severe-to-Healthy Reversals (0/122 instances):} In complete concordance with the clinical safety imperative, exactly zero Severe cases were misclassified into the Healthy category across any of the 4,008 non-training radiographs. For all 122 Severe instances, posterior probabilities for the Healthy class remained below $p_0 < 0.02$, with mean predicted Severe confidence exceeding 94.6\%. This empirical boundary containment validates that the expected-value ordinal penalty successfully eliminates catastrophic diagnostic omissions.
\end{enumerate}
...
Our experimental results demonstrate that the proposed framework achieved 100.00\% sensitivity (122/122) on the Severe cohort under dual-pass TTA, with zero observed instances of extreme grade reversals (0/122 Severe-to-Healthy misclassifications) across all five model configurations on the 4,008-image evaluation cohort. These empirical observations are consistent with the ordinal composite penalty contributing to the suppression of non-adjacent prediction errors by penalizing divergence proportionally to continuous distance in label space. When combined with dual-pass lateral test-time augmentation, prediction variance along borderline decision thresholds is substantially reduced, yielding a Quadratic Weighted Kappa of 0.853 that denotes ``almost perfect'' agreement according to the Landis--Koch scale \cite{landis1977measurement}. As delineated in Table~\ref{tab:comparison}, while direct accuracy comparisons must account for the difference between 3-class triage and 5-class nominal grading, both the primary held-out test baseline (83.44\% accuracy, 0.721 QWK, 0.943 AUC on $N = 1{,}656$) and the expanded multi-partition evaluation (90.92\% accuracy, 0.853 QWK, 0.980 AUC on $N = 4{,}008$) demonstrate competitive alignment with established radiologist-level benchmarks under strict patient-level leakage control.

\begin{table*}[t]
\centering
\caption{Comparison with published deep learning methods for KOA severity grading. Direct numerical comparisons must be interpreted with caution due to differing task granularities (5-class nominal KL grading vs.\ 3-class clinically actionable triage), splitting protocols (patient-level vs.\ image-level), and imaging views. Quadratic Weighted Kappa (QWK) provides the primary chance-corrected metric for ordinal agreement across differing scales.}
\label{tab:comparison}
\begin{tabular}{p{3.5cm}p{2.3cm}p{1.1cm}p{2.0cm}p{1.9cm}p{1.4cm}p{1.1cm}p{1.4cm}}
\toprule
...
Chen \textit{et al.} \cite{chen2019fully} & Ordinal DenseNet & 5 & OAI & Patient-level & --- & 0.79 & --- \\
Thomas \textit{et al.} \cite{thomas2020automated} & Attention CNN & 5 & OAI & Patient-level & 71.0\% & --- & --- \\
Swiecicki \textit{et al.} \cite{swiecicki2021deep} & Dual-View CNN & 5 & MOST & Patient-level & 71.9\% & 0.91 & --- \\
Mohammed \textit{et al.} \cite{bany2023knee} & ResNet-101 & 5 & Mendeley & Image-level & 74.8\% & --- & --- \\
Kondal \textit{et al.} \cite{kondal2023transfer} & Two-Stage CNN & 5 & OAI/Clinical & Patient-level & 72.3\% & --- & --- \\
\midrule
\textbf{Present study (Primary Test)} & \textbf{Hybrid (Static)} & \textbf{3} & \textbf{Mendeley ($N=1{,}656$)} & \textbf{Patient-level} & \textbf{83.44\%} & \textbf{0.721} & \textbf{0.943} \\
\textbf{Present study (Expanded)} & \textbf{Hybrid + TTA} & \textbf{3} & \textbf{Mendeley ($N=4{,}008$)} & \textbf{Patient-level} & \textbf{90.92\%} & \textbf{0.853} & \textbf{0.980} \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{enumerate}[leftmargin=*]
\item \textit{ConvNeXt-Base (Subchondral Trabecular Texture):} Depthwise separable convolutions operating over $7 \times 7$ receptive fields prove adept at resolving high-frequency subchondral bone eburnation and sharp marginal osteophytic spikes along the articular rim. Zeroing this branch induces a 7.32 percentage-point (pp) reduction in diagnostic accuracy, demonstrating that local trabecular density patterns provide indispensable diagnostic evidence.
\item \textit{Swin Transformer (Inter-Compartmental Relational Context):} Shifted-window self-attention captures long-range spatial dependencies across the articular gap, evaluating medial-to-lateral compartment height asymmetry and overall coronal joint alignment. Without this contextual perspective, classification accuracy drops by 4.99 pp.
\item \textit{DINO ViT-Base (Invariant Articular Geometry):} Preserving frozen self-supervised weights contributes an objective structural prior that resists overfitting on modest medical imaging cohorts. Ablating DINO produces the sharpest performance deterioration ($-7.64$ pp accuracy, $-0.113$ QWK), indicating that self-supervised distilled anatomical boundaries serve as an essential geometric anchor for the downstream classifier.
\end{enumerate}

\subsection{Translational Triage Architecture and Clinical Decision Support}
\label{subsec:clinical_workflow}

Translating artificial intelligence algorithms from academic benchmarks to clinical radiology requires seamless integration into hospital Picture Archiving and Communication Systems (PACS) and Electronic Health Record (EHR) environments. In contemporary healthcare systems, orthopedic outpatient clinics and emergency departments encounter substantial backlogs of plain radiographs, with delays of days to weeks before formal musculoskeletal radiologist interpretation \cite{braun2012diagnosis}. 

The proposed tri-paradigm framework provides a high-throughput, risk-calibrated triage engine capable of operating autonomously at the point of radiograph acquisition:
\begin{enumerate}[leftmargin=*]
\item \textit{Tier 0 (Healthy, KL 0--1):} Patients exhibiting preserved joint space and absence of definitive osteophytes can be safely scheduled for routine primary care follow-up or preventive physical activity counseling, avoiding unnecessary referrals to oversubscribed orthopedic specialists.
\item \textit{Tier 1 (Moderate, KL 2--3):} Identified cases with definitive osteophytosis and early-to-intermediate cartilage loss are automatically channeled to non-surgical joint preservation pathways, including targeted physical therapy, weight management, oral pharmacotherapy, and scheduled intra-articular corticosteroid or hyaluronic acid injections \cite{zhang2010eular}.
\item \textit{Tier 2 (Severe, KL 4):} Critically, by maintaining 100\% sensitivity with zero extreme grade reversals, advanced joint-failure cases are instantly prioritized for expedited orthopedic surgical consultation and total knee arthroplasty (TKA) pre-operative planning \cite{carr2012knee}, preventing prolonged functional decline.
\end{enumerate}
By functioning as an automated first-reader triage filter, the framework flags high-risk examinations for prioritized review, reducing diagnostic turnaround times and standardizing care delivery across resource-constrained healthcare settings.

\subsection{Sociodemographic Equity, Demographic Bias, and Algorithmic Fairness}
\label{subsec:bias_fairness}

A critical challenge confronting contemporary clinical AI is the propagation of latent demographic biases. Prior studies in musculoskeletal radiology have highlighted that underserved demographic cohorts often experience marked disparities in pain severity relative to radiographic KL grades \cite{pierson2021algorithmic}. Furthermore, knee osteoarthritis exhibits distinct epidemiological variations across biological sexes: post-menopausal women demonstrate significantly higher rates of accelerated medial compartment cartilage loss, severe subchondral bone remodeling, and self-reported physical disability compared to age-matched men \cite{hunter2019oa, loeser2012oa}.

In single-backbone CNN systems trained with nominal cross-entropy, models frequently exploit spurious radiopaque correlations, acquisition hardware artifacts, or soft-tissue thickness variations associated with high body mass index (BMI) rather than authentic joint pathology \cite{tajbakhsh2016convolutional}. In our proposed architecture, incorporating DINO's self-supervised visual priors provides invariant geometric representations of bone contours that are inherently decoupled from external exposure and patient habitus. Furthermore, as corroborated by our multi-method explainability suite (Figs.~\ref{fig:xai} and \ref{fig:lime}), model attributions strictly isolate the articular cartilage interface and subchondral plate, with zero attribution on peripheral soft tissue. Future multi-center evaluations must systematically assess algorithmic parity across diverse racial, ethnic, and socioeconomic demographics to guarantee equitable clinical deployment.
...
This investigation introduced an automated, leakage-free hybrid deep learning framework for knee osteoarthritis severity grading, integrating ConvNeXt-Base, Swin Transformer, and self-supervised DINO ViT through late feature fusion. Evaluated across 9,786 patient-stratified knee radiographs (4,130 unique subjects) with verified zero patient-level leakage, the framework achieves a baseline accuracy of 83.44\%, a Quadratic Weighted Kappa of 0.721, and a macro ROC-AUC of 0.943 on the strictly isolated primary held-out test partition ($N = 1{,}656$, 828 patients). Under dual-pass lateral test-time augmentation on the expanded 4,008-image multi-partition stress cohort, the Hybrid model attains 90.92\% accuracy, a QWK of 0.853, and a macro ROC-AUC of 0.980. Critically, the Severe cohort achieved 100\% sensitivity (122/122) with zero observed extreme grade reversals into Healthy across all evaluated models. Qualitative explainability analysis across seven gradient and perturbation attribution methods confirmed consistent anatomical grounding on the tibiofemoral joint margins. Future investigations will prioritize domain-specific pretraining on musculoskeletal radiographs, multimodal clinical feature integration, and multi-center prospective validation.
...
M.~C. Nevitt, D.~T. Felson, and G.~Lester, ``The Osteoarthritis Initiative: Protocol for the cohort study,'' 2006.
\end{thebibliography}
