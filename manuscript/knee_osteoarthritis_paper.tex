...
% ==============================================================================
\begin{abstract}
Radiographic assessment of knee osteoarthritis (KOA) via the Kellgren--Lawrence (KL) scale is hindered by high inter-observer subjectivity, bilateral data leakage in automated pipelines, and the neglect of ordinal disease progression. This investigation establishes a clinically oriented hybrid deep learning architecture that combines three complementary visual paradigms: ConvNeXt-Base for localized subchondral bone texture, Swin Transformer for shifted-window global compartment modeling, and DINO Vision Transformer for self-supervised structural priors. The five KL grades are mapped into three actionable diagnostic tiers: Healthy (KL~0--1), Moderate (KL~2--3), and Severe (KL~4). To mitigate catastrophic diagnostic errors, an expected-value ordinal penalty loss is formulated that dynamically penalizes non-adjacent grade misclassifications during training. Evaluated across 9,786 patient-stratified knee radiographs (4,130 unique subjects) with verified zero patient-level leakage, the framework was assessed on a 4,008-image evaluation cohort. Combining dual-pass lateral-reflection test-time augmentation with soft-voting ensemble fusion, the Hybrid Fusion model attains 90.92\% accuracy, a Quadratic Weighted Kappa of 0.853, and a macro ROC-AUC of 0.980. Crucially, the Severe tier achieves 100\% sensitivity with zero extreme grade reversals (Severe classified as Healthy). Interpretability analysis across seven gradient- and perturbation-based methods confirms consistent anatomical grounding on tibiofemoral joint-space narrowing and marginal osteophytes, demonstrating strong potential for clinical screening and triage.
\end{abstract}
...
\begin{figure*}[!t]
    \centering
    \includegraphics[width=0.98\linewidth]{fig1.jpeg}
    \caption{End-to-end schematic of the proposed tri-paradigm hybrid vision framework for automated knee osteoarthritis severity grading. A standardized anteroposterior radiograph ($\mathbf{X} \in \mathbb{R}^{224 \times 224 \times 3}$) is concurrently processed under dual-pass lateral test-time augmentation through three specialized backbones: ConvNeXt-Base for localized subchondral bone texture ($d_1=1024$), Swin-Base Transformer for shifted-window joint compartment context ($d_2=1024$), and self-supervised DINO ViT-Base for invariant structural priors ($d_3=768$). The concatenated representation ($\mathbf{z}_{\text{fused}} \in \mathbb{R}^{2816}$) is projected through a dual-stage regularized multi-layer perceptron head with batch normalization and dropout, yielding calibrated posterior probabilities across three clinically actionable diagnostic tiers (Healthy/Mild KL~0--1, Moderate KL~2--3, and Severe KL~4) optimized via an ordinal-penalized composite objective.}
...
\begin{figure*}[!t]
    \centering
    \includegraphics[width=0.98\linewidth]{fig2.jpeg}
...
\begin{figure*}[!t]
    \centering
    \includegraphics[width=0.98\linewidth]{fig3.jpeg}
...
\begin{figure*}[!t]
    \centering
    \includegraphics[width=0.98\linewidth]{fig4.jpeg}
...
\begin{figure*}[!t]
    \centering
    \includegraphics[width=0.98\linewidth]{fig5.jpeg}
...
A key question in medical computer vision is whether multi-backbone fusion provides genuine representation complementarity or merely redundant parameter scaling. Our branch ablation results (Table~\ref{tab:ablation}) demonstrate that each backbone captures distinct radiographic features:

\begin{enumerate}[leftmargin=*]
\item \textit{ConvNeXt-Base (Local Texture Encoding):} Modern depthwise convolutions with $7 \times 7$ receptive fields effectively encode high-frequency subchondral bone eburnation and sharp marginal osteophytes. Ablating this branch caused a 7.32\% drop in classification accuracy.
\item \textit{Swin Transformer (Global Contextual Attention):} The shifted-window self-attention mechanism models spatial relationships between the medial and lateral compartments, capturing overall joint alignment and asymmetry.
\item \textit{DINO ViT-Base (Self-Supervised Structural Priors):} Retaining frozen self-supervised weights prevents overfitting on modest clinical datasets while contributing robust boundary-detection priors. Ablating the DINO branch resulted in the largest performance decline ($-7.64\%$ accuracy, $-0.113$ QWK), confirming that self-supervised pretraining provides essential structural representations that complement supervised representations.
\end{enumerate}
