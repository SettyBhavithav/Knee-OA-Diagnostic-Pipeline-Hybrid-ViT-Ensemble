...
In addition to the core 8,260-image classification set, the Mendeley repository includes a supplementary benchmark partition of 1,526 detector-localized knee joint crops (\texttt{auto\_test}) derived from 811 subjects belonging strictly to the primary held-out test cohort ($\mathcal{P}_{\text{Auto}} \subset \mathcal{P}_{\text{Test}}$) \cite{chen2018dataset, chen2019fully}. The supplementary Auto-Test crops are provided in the benchmark release and represent detector-localized knee crops, introducing localized boundary shifts and field-of-view perturbations. We evaluated this supplementary partition under verified zero-training-leakage conditions ($|\mathcal{P}_{\text{Train}} \cap \mathcal{P}_{\text{Auto}}| = 0$) to measure model resilience against automated detection noise.
...
\item \textbf{Severe OA ($\mathcal{Y}_2$, KL~4):} Advanced joint collapse characterized by extensive osteophytosis, complete joint-space obliteration, and severe subchondral sclerosis, identifying advanced radiographic disease that may warrant specialist clinical evaluation \cite{carr2012knee}.
\end{itemize}
