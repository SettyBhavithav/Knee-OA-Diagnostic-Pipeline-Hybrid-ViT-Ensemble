...
Conventional medical image classifiers typically minimize standard categorical cross-entropy (CE) loss, which implicitly treats disease categories as nominal and mutually orthogonal classes. Under standard CE, a severe diagnostic error (e.g., misclassifying Severe OA [KL~4] as Low/No OA [KL~0--1]) incurs an identical penalty to an adjacent boundary disagreement (e.g., classifying Low/No OA [KL~0--1] as Moderate OA [KL~2--3]). In clinical musculoskeletal practice, Kellgren--Lawrence grading establishes an ordered radiographic severity scale reflecting progressively greater structural joint degeneration, characterized by advancing articular cartilage loss, joint-space narrowing, and subchondral bone deformation \cite{kellgren1957radiological, emrani2008joint}. Non-adjacent severity misclassification may have greater clinical implications than disagreement between adjacent radiographic stages, particularly when severe disease is assigned to the lowest severity tier \cite{zhang2010eular}.
...
The radiographic cohort analyzed in this study is sourced from the Knee Osteoarthritis Severity Grading Dataset published by Chen~\textit{et~al.} on Mendeley Data (Version~1, DOI: 10.17632/56rmx5bjcr.1) \cite{chen2018dataset}, which compiles bilateral weight-bearing knee radiographs acquired under standardized protocols from the Osteoarthritis Initiative (OAI) longitudinal study \cite{nevitt2006oai}. The complete repository contains 9,786 radiographic image files, including the core radiographs and supplementary detector-localized knee crops.
...
In addition to the core 8,260-image classification set, the Mendeley repository includes a supplementary benchmark partition of 1,526 knee joint crops (\texttt{auto\_test}) derived from 811 subjects belonging strictly to the primary held-out test cohort ($\mathcal{P}_{\text{Auto}} \subset \mathcal{P}_{\text{Test}}$) \cite{chen2018dataset}. The benchmark release additionally provides supplementary knee joint crops, which we use as a secondary evaluation condition to measure model resilience against localized boundary shifts and field-of-view perturbations under verified zero-training-leakage conditions ($|\mathcal{P}_{\text{Train}} \cap \mathcal{P}_{\text{Auto}}| = 0$).
...
\vspace{2pt}
\parbox{\columnwidth}{\footnotesize $^*$Auto-Test images represent supplementary knee joint crops provided in the Mendeley benchmark release \cite{chen2018dataset}, derived from 811 subjects belonging strictly to the primary held-out test cohort ($\mathcal{P}_{\text{Auto}} \subset \mathcal{P}_{\text{Test}}$). They maintain verified patient-level isolation from training and validation partitions ($|\mathcal{P}_{\text{Train}} \cap \mathcal{P}_{\text{Auto}}| = |\mathcal{P}_{\text{Val}} \cap \mathcal{P}_{\text{Auto}}| = 0$).}
\end{table}
...
Standard categorical cross-entropy treats the three diagnostic tiers nominally, assigning equal loss regardless of whether a Severe OA case is misclassified as Moderate OA or Low/No OA. To reduce the likelihood of large ordinal misclassification errors, we formulate an ordinal risk objective that incorporates continuous metric distance in label space.
