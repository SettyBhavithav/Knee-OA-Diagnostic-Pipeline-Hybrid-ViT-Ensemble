...
Nevertheless, an appraisal of the literature highlights three methodological vulnerabilities. First, several investigations partition benchmark datasets at the image level rather than the patient level. Because bilateral radiographs from the same individual share genetic bone morphology, anatomical habitus, and exposure parameters, random image splitting introduces acute data leakage across training and testing partitions, artificially inflating reported accuracies \cite{tajbakhsh2016convolutional}. Second, prevailing models optimize standard nominal cross-entropy loss, which penalizes all classification errors uniformly. Clinically, confusing Severe OA (KL~4, advanced joint failure) with Low/No OA (KL~0--1) represents a hazardous omission, whereas misclassifying an adjacent transition (KL~1 vs.\ KL~2) carries manageable clinical consequences \cite{antony2016quantifying}. Third, single-backbone architectures exhibit intrinsic inductive bias trade-offs: standard convolutional neural networks (CNNs) prioritize localized high-frequency edge textures at the expense of long-range spatial context, whereas vision transformers (ViTs) require extensive pretraining to establish spatial invariance without inductive priors.
...
Pairwise set intersections confirm zero subject overlap across all partitions:
\begin{equation}
...
In addition to the core 8,260-image classification set, a supplementary partition of 1,526 detector-localized knee joint crops (Auto-Test) was generated from 811 patients belonging strictly to the held-out test partition ($\mathcal{P}_{\text{Auto}} \subset \mathcal{P}_{\text{Test}}$). These crops were extracted using an independent YOLOv5 joint detection model trained on external knee bounding boxes \cite{chen2018dataset} at $640 \times 640$ resolution (confidence threshold: 0.60, NMS IoU threshold: 0.45) with 10\% margin padding to evaluate model sensitivity to localized field-of-view perturbations under verified zero-leakage conditions ($|\mathcal{P}_{\text{Train}} \cap \mathcal{P}_{\text{Auto}}| = 0$).
...
\end{tabular}
\vspace{2pt}
\parbox{\columnwidth}{\footnotesize $^*$Auto-Test images represent automated YOLO-based knee joint detector crops extracted from 811 subjects belonging strictly to the primary held-out test cohort ($\mathcal{P}_{\text{Auto}} \subset \mathcal{P}_{\text{Test}}$). They maintain complete patient-level isolation from training and validation partitions ($|\mathcal{P}_{\text{Train}} \cap \mathcal{P}_{\text{Auto}}| = |\mathcal{P}_{\text{Val}} \cap \mathcal{P}_{\text{Auto}}| = 0$).}
\end{table}

\subsection{Clinically Actionable Diagnostic Tiers}
\label{subsec:class_remapping}

To align automated prediction with frontline orthopedic triage pathways, the five nominal KL grades were consolidated into three clinically actionable diagnostic tiers:
\begin{itemize}[leftmargin=*]
...
\item \textbf{Severe OA ($\mathcal{Y}_2$, KL~4):} Advanced joint collapse characterized by extensive osteophytosis, complete joint-space obliteration, and severe subchondral sclerosis, identifying individuals warranting specialist evaluation for surgical joint reconstruction \cite{carr2012knee}.
\end{itemize}
...
\begin{tabular}{lcccc}
\toprule
\multirow{2}{*}{\textbf{Diagnostic Class}} & \multicolumn{2}{c}{\textbf{Held-Out Test ($N=1{,}656$)}} & \multicolumn{2}{c}{\textbf{Composite Cohort ($N=4{,}008$)}} \\
...
\item \textbf{Reproducibility Protocol and Checkpoint Selection:} Global random seeds were locked across Python \texttt{random}, NumPy, PyTorch, and CUDA backend (\texttt{torch.backends.cudnn.deterministic = True}) using seed 42. Terminal model weights were selected strictly based on the highest validation Quadratic Weighted Kappa (QWK) on $\mathcal{P}_{\text{Val}}$.
\end{itemize}
...
\label{eq:ensemble}
\end{equation}
where weights $\mathbf{w} = [0.10, 0.20, 0.10, 0.60]$ were calibrated strictly on the validation partition ($N = 826$ images, 413 patients), granting dominant influence to the tri-paradigm hybrid model while maintaining the primary held-out test partition untouched for unbiased evaluation.

\subsection{Multi-Branch Explainability Suite}
\label{subsec:xai}

To audit model fidelity against clinical biomarkers in alignment with radiology interpretability principles \cite{reyes2020interpretability}, seven complementary methods are implemented:
\begin{enumerate}[leftmargin=*, label=(\roman*)]
...
On this expanded cohort, Hybrid Fusion achieves the highest overall accuracy at 90.92\% (95\% CI: [90.02\%, 91.82\%]), outperforming the Soft-Voting Ensemble (90.54\%) and standalone DINO ViT (90.22\%). Standalone ConvNeXt-Base achieves 87.52\%, while Swin Transformer attains 84.26\%. The QWK of 0.853 reflects substantial ordinal concordance according to the Landis--Koch scale \cite{landis1977measurement} across the three diagnostic tiers, while the macro ROC-AUC of 0.980 confirms discriminative capacity across multi-partition inputs.
...
\item \textbf{Complete Sensitivity for Advanced OA:} Severe OA sensitivity reached 100.00\% (122/122 correct detections, 95\% CI: [97.0\%, 100.0\%]), with a precision of 94.57\% and F1-score of 97.21\%. No Severe instance was misclassified into the Low/No OA category across any evaluated model, confirming the suppression of non-adjacent errors under the distance-aware penalty.
...
For the Low/No OA tier ($N = 2{,}295$), the proposed Hybrid Fusion model attains a diagnostic recall of 94.60\%, correctly identifying 2,171 instances, with all 124 residual errors confined to the borderline Moderate tier and zero into Severe OA. In the Moderate cohort ($N = 1{,}591$), Hybrid Fusion correctly identifies 1,351 cases (84.92\% recall), with 234 misclassified as Low/No OA (14.71\%) and 6 misclassified as Severe (0.38\%). Both error directions remain strictly adjacent to the ground-truth tier, yielding a total Moderate error rate of 15.08\% (240/1,591). The Soft-Voting Ensemble demonstrates comparable consistency, correctly identifying 2,165 Low/No OA cases (94.34\% recall), 1,327 Moderate cases (83.41\% recall; with 257 assigned to Low/No OA [16.15\%] and 7 assigned to Severe [0.44\%]), and 122 Severe cases (100.00\% recall). This empirical distribution verifies that the distance-aware ordinal penalty suppresses non-adjacent prediction errors, concentrating residual uncertainty entirely along the clinically ambiguous KL~1 versus KL~2 transition boundary.
...
The Ensemble significantly outperformed Swin Transformer ($\chi^2 = 31.53, p < 10^{-4}$). The comparison with DINO ViT yielded $p = 0.0528$, which did not reach the prespecified significance threshold under raw or Holm-adjusted criteria ($\alpha_{\text{Holm}} = 0.0167$). Differences between ConvNeXt, Hybrid Fusion, and the Ensemble were not statistically significant, confirming that the hybrid feature representation captures the primary discriminative signal.
...
The ablation confirms the necessity of distance-aware optimization: standard cross-entropy achieves 81.28\% accuracy and 0.678 QWK, but incurs 2 catastrophic non-adjacent reversals (Severe misdiagnosed as Low/No OA). Incorporating the distance penalty ($\lambda_{\text{ord}} = 0.50$) improves accuracy by +2.16 pp to 83.44\%, elevates QWK by +0.043 to 0.721, achieves 100\% Severe recall on held-out test data, and completely eliminates extreme grade reversals. Increasing the weight to $\lambda_{\text{ord}} = 1.00$ slightly over-penalizes metric distance at the expense of nominal boundary precision (82.61\% accuracy), confirming $\lambda_{\text{ord}} = 0.50$ as the optimal trade-off parameter.
...
\toprule
\textbf{Architecture} & \textbf{Parameters (M)} & \textbf{FLOPs (G)} & \textbf{GPU Latency (ms)} & \textbf{Throughput (img/s)} \\
\midrule
ConvNeXt-Base & 88.6 & 15.4 & $4.6 \pm 0.3$ & 217.4 \\
Swin-Base & 87.8 & 15.4 & $5.1 \pm 0.4$ & 196.1 \\
DINO ViT-Base & 86.6 & 17.6 & $5.8 \pm 0.4$ & 172.4 \\
MLP Projection Head & 3.41 & 0.01 & $0.3 \pm 0.05$ & >2000 \\
\midrule
\textbf{Full Hybrid (Static)} & \textbf{266.4} & \textbf{48.4} & $\mathbf{14.2 \pm 0.8}$ & \textbf{70.4} \\
\textbf{Full Hybrid (+ TTA)} & \textbf{266.4} & \textbf{96.8} & $\mathbf{26.8 \pm 1.2}$ & \textbf{37.3} \\
\bottomrule
\end{tabularx}
\vspace{2pt}
\parbox{\columnwidth}{\footnotesize Latencies measured on an NVIDIA GeForce RTX 4050 Laptop GPU (6~GB GDDR6, CUDA 12.1, PyTorch 2.4.0) with batch size $B = 16$ under mixed precision (FP16) after 10 warmup iterations, averaged over 100 synchronized runs. Throughput denotes $B / \text{latency}_{\text{batch}}$.}
\end{table}

The tri-paradigm Hybrid architecture consolidates 266.4 million total parameters: 88.6M allocated to ConvNeXt-Base, 87.8M to Swin-Base, 86.6M to frozen DINO ViT-Base, and 3.41M to the dual-stage MLP projection head ($2816 \to 1024 \to 512 \to 3$ with Batch Normalization, totaling 3,414,019 parameters). Forward execution across the three parallel backbones incurs 48.4 GFLOPs per single-pass radiograph. On an NVIDIA GeForce RTX 4050 GPU under mixed precision (FP16), single-pass static inference requires $14.2 \pm 0.8\text{ ms}$ per image, corresponding to a throughput of 70.4 images per second. Under horizontal-flip TTA, total computation scales to 96.8 GFLOPs, requiring $26.8 \pm 1.2\text{ ms}$ per radiograph ($37.3\text{ images/s}$). On host CPU execution (Intel Core i7-13700H @ 2.40~GHz), single-pass latency averaged $118.5 \pm 4.6\text{ ms}$. Peak GPU memory consumption during batch inference ($B=16$) remained below 2.2~GB. These measurements indicate favorable computational efficiency for potential integration into clinical decision-support workflows.
...
\begin{enumerate}[leftmargin=*]
\item \textit{ConvNeXt-Base (Subchondral Trabecular Texture):} Depthwise separable convolutions operating over $7 \times 7$ receptive fields capture high-frequency subchondral bone eburnation and marginal osteophytic contours along the articular rim. Zeroing this branch produces a 7.32 percentage-point reduction in diagnostic accuracy, demonstrating that local trabecular density patterns provide substantial diagnostic evidence.
\item \textit{Swin Transformer (Inter-Compartmental Relational Context):} Shifted-window self-attention models spatial dependencies across the articular gap, comparing medial-to-lateral compartment height asymmetry and overall joint alignment. Without this contextual perspective, classification accuracy drops by 4.99 pp.
\item \textit{DINO ViT-Base (Invariant Articular Geometry):} Preserving frozen self-supervised weights provides an objective structural prior that resists overfitting on modest medical cohorts. Perturbing DINO produces the sharpest performance drop ($-7.64$ pp accuracy, $-0.113$ QWK), indicating that self-supervised distilled anatomical boundaries serve as an essential geometric anchor.
\end{enumerate}

\subsection{Translational Triage Architecture and Clinical Decision Support}
\label{subsec:clinical_workflow}

Translating computer-aided detection models into radiology environments requires seamless integration into hospital Picture Archiving and Communication Systems (PACS) and Electronic Health Record (EHR) systems \cite{braun2012diagnosis}. The proposed tri-paradigm framework is designed as a computer-aided triage filter to support prioritization of radiograph worklists:
...
\item \textit{Tier 1 (Moderate OA, KL~2--3):} Radiographs with definitive osteophytosis and intermediate joint space loss can be flagged for clinician review to consider non-surgical joint preservation pathways (targeted physical therapy, weight management, oral pharmacotherapy, and scheduled intra-articular injections) \cite{zhang2010eular}.
...
A recognized challenge in musculoskeletal machine learning is the potential presence of demographic disparities. Prior studies have observed that underserved patient cohorts may experience substantial discordance between reported pain severity and radiographic KL grades \cite{pierson2021algorithmic}. Furthermore, knee osteoarthritis exhibits epidemiological variations across biological sexes, with post-menopausal women exhibiting higher rates of medial compartment cartilage loss and functional impairment \cite{hunter2019oa, loeser2012oa}.

Single-backbone models trained with nominal cross-entropy can inadvertently exploit non-pathological correlations, such as soft-tissue thickness variations associated with patient body habitus or beam exposure discrepancies \cite{tajbakhsh2016convolutional}. In our proposed architecture, incorporating DINO's self-supervised visual representations may reduce sensitivity to non-pathological variations by emphasizing intrinsic structural contours. Furthermore, qualitative explainability audits (Figs.~\ref{fig:xai} and \ref{fig:lime}) confirm that attributions concentrate along the tibiofemoral articular interface. However, no formal demographic subgroup fairness audit was conducted in this study due to the absence of linked sociodemographic metadata in the public imaging repository. This represents an important limitation, and future multi-center evaluations must systematically validate performance across diverse patient populations.
...
\begin{enumerate}[leftmargin=*]
\item \textit{Single Benchmark Source:} Although the dataset comprises 9,786 radiographs across 4,130 unique subjects, the underlying images originate from the Osteoarthritis Initiative (OAI) cohort \cite{nevitt2006oai}. Evaluation across external imaging centers with varied detector hardware and exposure protocols remains necessary.
...
\item \textit{Three-Tier Consolidation:} Grouping KL grades into Low/No OA (KL~0--1), Moderate OA (KL~2--3), and Severe OA (KL~4) reflects triage management categories, but does not provide granular discrimination between KL~2 and KL~3 for monitoring multi-year structural progression.
\end{enumerate}
...
\item \textit{Federated Multi-Center Validation:} Deploying models within federated learning consortia to enable multi-institutional clinical validation while preserving patient privacy.
\end{enumerate}
